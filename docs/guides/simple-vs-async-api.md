...
1. **`pupil_labs.realtime_api.simple.Device`**
   Easy-to-use class that auto-connects to the Companion device on `__init__` to
   request the [current status](simple_get_status_example) and to keep it up-to-
...
2. **`pupil_labs.realtime_api.models.DiscoveredDeviceInfo`**
...
3. **`pupil_labs.realtime_api.device.Device`**
   ...
   the `Websocket API <pupil_labs.realtime_api.device.StatusUpdateNotifier>`,
   [streaming](streaming_api), and `clock offset
