...
class Device(DeviceBase):
    ...
    async def recording_stop_and_save(self) -> str:
        ...
        async with self.active_session.post(
            ...
            if response.status != 200:
                raise DeviceError(response.status, confirmation.get("message"))
            if confirmation.get("result"):
                return cast(str, confirmation["result"]["id"])
            else:
                return cast(str, confirmation.get("message"))

    async def recording_cancel(self) -> str:
    ...
    async def get_template_data(
        self, data_format: TemplateDataFormat = "simple"
    ) -> Any:
        """Get the template data entered on device.

        Args:
            data_format (TemplateDataFormat): Format of the returned data.
                - "api" returns the data as is from the api e.g., {"item_uuid": ["42"]}
                - "simple" returns the data parsed e.g., {"item_uuid": 42}

        Returns:
            dict: The template data in the requested format.
        ...
        """
        assert data_format in TemplateDataFormat.__args__, (  # type: ignore[attr-defined]
    ...
    async def post_template_data(
        ...
        pre_populated_data = await self.get_template_data(data_format="api")
        errors = self.template_definition.validate_answers(
            pre_populated_data | template_answers, data_format="api"
        )
        if errors:
