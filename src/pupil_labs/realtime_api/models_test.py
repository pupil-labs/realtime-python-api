...
def test_template_answer():
    ...
    # fmt: on
    template_def.validate_answers(good_template_answers)

    # fmt: off
    bad_number_answers = good_template_answers | {
    ...
    with pytest.raises(InvalidTemplateAnswersError) as exc:
        template_def.validate_answers(bad_number_answers)
        assert len(exc.errors) == 4
        for error in exc.errors:
            assert "unable to parse" in error["msg"]

    # fmt: off
    missing_choice_answers = good_template_answers | {
    ...
    errors = template_def.validate_answers(
        missing_choice_answers, raise_exception=False
    )
    assert len(errors) == 6
    for error in errors:
    ...
    # fmt: off
    too_many_answers = good_template_answers | {
    ...
    errors = template_def.validate_answers(too_many_answers, raise_exception=False)
    assert len(errors) == 4
    for error in errors:
