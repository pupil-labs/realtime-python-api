...
class Device(DeviceBase):
    ...
    @property
    def memory_free_gb(self) -> str:
        return self._status.phone.memory_in_gb
    ...
    def recording_stop_and_save(self) -> str:
    ...
    def recording_cancel(self) -> str:
        ...
        async def _cancel_recording() -> str:
            async with _DeviceAsync.convert_from(self) as control:
    ...
    def get_template_data(self, data_format: TemplateDataFormat = "simple") -> Any:
        ...
        async def _get_template_data() -> Any:
            async with _DeviceAsync.convert_from(self) as control:
                return await control.get_template_data(data_format=data_format)
        ...
        async def _post_template_data() -> Any:
            async with _DeviceAsync.convert_from(self) as control:
                return await control.post_template_data(
                    template_data, data_format=data_format
                )
    ...
    def receive_eyes_video_frame(
        ...
        """
        return cast(
            SimpleVideoFrame, self._receive_item(SensorName.EYES.value, timeout_seconds)
        )
    ...
    def receive_eye_events(
        self, timeout_seconds: float | None = None
    ) -> FixationOnsetEventData | FixationOnsetEventData | BlinkEventData | None:
        return cast(
            FixationOnsetEventData | FixationOnsetEventData | BlinkEventData,
    ...
    def _receive_item(
        ...
        try:
            return cast(ReceivedItemType, self._most_recent_item[sensor].popleft())
        except IndexError:
    ...
    def streaming_start(self) -> None:
        ...
        This starts streaming for all available sensors.
        """
        self._streaming_trigger_action(self._EVENT.SHOULD_STREAMS_START)

    def streaming_stop(self) -> None:
        """Stop streaming data from the device.

        This stops streaming for all sensors.
        """
