...
class _AsyncEventManager(Generic[EventKey]):
    ...
    def trigger_threadsafe(
        ...
        # Potential KeyError is implicitly handled by accessing self._events[name]
        # inside the lambda/partial used by call_soon_threadsafe, but occurs
        # in the loop's thread. Consider adding explicit check if needed earlier
    ...
    async def wait_for_first_event(self) -> EventKey:
        ...
        try:
            ...
            for task in tasks:
                task.cancel()
            # Wait for cancellations to propagate before raising
...
class _StreamManager:
    ...
    def _stop_streaming_task_if_running(self) -> None:
        ...
        if self._streaming_task is not None:
            sensor_name = (
                self._recent_sensor.sensor if self._recent_sensor else "unknown"
            )
    ...
    async def append_data_from_sensor_to_queue(self, sensor: Sensor) -> None:  # noqa: C901
        ...
        manager's `_streaming_cls`. It receives items (gaze data or video frames),
    ...
    def _get_closest_item(
        ...
        if not cache:  # Check if cache is empty
