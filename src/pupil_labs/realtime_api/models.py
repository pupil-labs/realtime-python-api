...
class Event(NamedTuple):
    ...
    def from_dict(cls, dct: dict[str, Any]) -> "Event":
        ...
        Args:
            dct: Dictionary containing event data.
...
class Phone(NamedTuple):
    """Information about the phone connected to the device.
    ...
    @property
    def memory_in_gb(self) -> str:
        """Return available memory in GB as a formatted string."""
        return f"{self.memory / 1024**3:.2f}GB"
...
class NetworkDevice(NamedTuple):
    """Information about devices discovered by the host device.
...
class Recording(NamedTuple):
    ...
    @property
    def rec_duration_hms(self) -> str:
        duration = timedelta(seconds=self.rec_duration_seconds)
        return str(duration)
...
class Status:
    ...
    def update(self, component: Component) -> None:
        """Update status with a new component.

        Args:
            component: New component information to update.
    ...
    def direct_world_sensor(self) -> Sensor | None:
        ...
        On Pupil Invisible devices, the world camera can be detached
        """
        return next(
    ...
    def direct_eye_events_sensor(self) -> Sensor | None:
        ...
        Only available on Neon with Companion App version 2.8.49 or newer.
        """
        return next(
...
class TemplateItem:
    ...
    Attributes:
        ...
        help_text (str | None): Help text for the item.
    ...
    def validate_answer(
        self,
        answer: Any,
        data_format: TemplateDataFormat = "simple",
        ...
        Args:
            ...
            data_format: Format of the answer ("simple" or "api").
        ...
        """
        validator = self._pydantic_validator(data_format=data_format)
    ...
    def _pydantic_validator(
        self, data_format: TemplateDataFormat
    ) -> tuple[type, FieldInfo] | None:
        """Create a Pydantic validator for this item.

        Args:
            data_format: Format of the data to validate ("simple" or "api").
    ...
    def _simple_model_validator(self) -> tuple[type, FieldInfo]:
        """Create a validator for "simple" format.
    ...
    def _api_model_validator(self) -> tuple[type, FieldInfo]:
        ...
        if self.widget_type in {"RADIO_LIST", "CHECKBOX_LIST"}:
            if not self.choices:  # Choices would be defined but mypy complains
                logging.warning(
                    f"Choices are not defined for widget type {self.widget_type}"
                )
...
class Template:
    ...
    def _create_answer_model(self, data_format: TemplateDataFormat) -> type[BaseModel]:
        """Create a Pydantic model for validating answers.

        Args:
            data_format: Format of the answers ("simple" or "api").
        ...
        for question in self.items:
            validator = question._pydantic_validator(data_format=data_format)
            if validator is None:
                continue
    ...
    def validate_answers(
        ...
        raise_exception: bool = True,
        data_format: TemplateDataFormat = "api",
    ) -> list[ErrorDetails]:
        ...
        """
        AnswerModel = self._create_answer_model(data_format=data_format)
        errors = []
        try:
            AnswerModel(**answers)
        except ValidationError as e:
