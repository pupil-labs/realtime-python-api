...
class RTSPData(NamedTuple):
    ...
    Attributes:
        ...
        timestamp_unix_seconds (float): Timestamp in seconds since the Unix epoch.
    ...
    def timestamp_unix_ns(self) -> int:
        """Get the timestamp converted to nanoseconds Unix epoch (int)."""
...
class RTSPRawStreamer:
    """Stream raw data from an RTSP source with accurate timestamps.
    ...
    async def __aexit__(self, *args: Any, **kwargs: Any) -> Any:
        ...
        Returns:
            Any: Result from the reader's __aexit__ method.
...
class _WallclockRTSPReader(AiortspRTSPReader):
    ...
    def absolute_timestamp_from_packet(self, packet: Any) -> float:
        """Get the absolute timestamp (Unix epoch) for an RTP packet.

        This method converts the relative RTP timestamp to an absolute Unix epoch
        timestamp using the clock offset determined from RTCP SR packets.
        ...
        try:
            return (
                ...
                + float(self._relative_to_ntp_clock_offset)
                if self._relative_to_ntp_clock_offset is not None
                else float(0)
            )
        except TypeError:
    ...
    def relative_timestamp_from_packet(self, packet: Any) -> float:
        ...
        try:
            ...
            clock_rate = rtpmap.get("clockRate")
            if (
                clock_rate is None
                or not isinstance(clock_rate, (int, float))
                or clock_rate <= 0
            ):
                raise SDPDataNotAvailableError(  # noqa: TRY301
                    f"Invalid or missing 'clockRate' in rtpmap: {clock_rate}"
                )
            packet_ts = getattr(packet, "ts", None)
            if packet_ts is None or not isinstance(packet_ts, (int, float)):
                raise AttributeError(
                    f"Packet object missing a valid 'ts' attribute (value: {packet_ts})"
                )
        except SDPDataNotAvailableError:
            raise
        except (KeyError, TypeError) as e:
            raise SDPDataNotAvailableError(
                f"SDP data is missing or incomplete: {e}"
            ) from e
        else:
            return float(packet_ts / clock_rate)
